...
def test_rc14_product_pages_are_launch_ready_drafts() -> None:
    ...
    assert len(product_files) == 20
    for path in product_files:
        text = _read(path)
        assert "## Product promise" in text
...
def test_rc14_seo_pages_have_complete_landing_page_sections() -> None:
    seo_files = sorted((CONTENT / "seo").glob("*.md"))

    assert len(seo_files) == 25
...
def test_rc14_big_cats_ebook_package_is_complete() -> None:
    ...
    assert {path.name for path in ebook.glob("*.md")} == expected
...
def test_rc14_readiness_report_declares_scores_and_scope() -> None:
    ...
    assert "Educational readiness | 92 / 100" in report
    assert "Publication readiness | 74 / 100" in report
    assert "Rights readiness | 58 / 100" in report
    assert "Product readiness | 81 / 100" in report
